...
export function usePagedReaderLayout({
  ...
  const attemptPagedRestore = useCallback((params: {
    currentPageIndex: number;
    nextPageCount: number;
    pendingRestoreTarget: ReaderRestoreTarget;
  }): 'handled' | 'pending' => {
    const { currentPageIndex, nextPageCount, pendingRestoreTarget } = params;
  ...
  useEffect(() => {
    ...
    if (hasRestorableTarget && pendingRestoreTarget && currentPagedLayout) {
    ...
    if (
      ...
      } else if (!pagedViewportSize.width || !pagedViewportSize.height) {
