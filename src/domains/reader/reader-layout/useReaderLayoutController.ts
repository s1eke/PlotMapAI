...
import type { ReaderAnalysisBridgeState } from '../reader-analysis-bridge';
import type { UseReaderChapterDataResult } from '../hooks/useReaderChapterData';
...
import type { UseReaderPreferencesResult } from '../hooks/useReaderPreferences';
import type { UseReaderRestoreFlowResult } from '../hooks/useReaderRestoreFlow';
import type {
...
import type { UseReaderSessionResult } from '../reader-session';
...
interface UseReaderLayoutControllerParams {
  ...
  preferences: Pick<
    ...
    | 'readerTheme'
  >;
  restoreFlow: UseReaderRestoreFlowResult;
  session: Pick<UseReaderSessionResult, 'commands' | 'snapshot'>;
  uiBridge: ReaderUiBridgeValue;
}
...
export function useReaderLayoutController({
  ...
  const { restoreSettledHandlerRef } = uiBridge;
