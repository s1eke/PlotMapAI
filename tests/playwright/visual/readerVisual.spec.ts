...
test.describe('阅读器视觉回归', () => {
  test('TC-055 富文本滚动视口基线渲染正确', async ({ page }) => {
  ...
  test('TC-056 分页模式（水平滑动转场）的阅读器基线渲染正确', async ({ page }) => {
  ...
  test('TC-057 图片查看器遮罩层基线渲染正确', async ({ page }) => {
  ...
  test('TC-058 预置章节分析数据时摘要视图基线渲染正确', async ({ page }) => {
  ...
  test('TC-059 通过阅读流程正确渲染导入的分隔线、内部链接和简单表格', async ({ page }) => {
  ...
  test('TC-060 多图片章节的画廊间距渲染稳定', async ({ page }) => {
  ...
  test('TC-061 滚动模式下纸张主题首屏语义展示渲染正确', async ({ page }) => {
  ...
  test('TC-062 文本策略富文本首屏渲染稳定', async ({ page }) => {
  ...
  test('TC-063 窄栏翻页标题按测量行渲染稳定', async ({ page }) => {
    await importFixtureToDetailPage(page, 'textPolicyShowcase');
    await setReaderPreferences(page, {
      fontSize: 20,
      lineSpacing: 1.6,
  ...
  test('TC-064 文本策略图注与表格富文本行渲染稳定', async ({ page }) => {
    await importFixtureToDetailPage(page, 'textPolicyShowcase');
    await openReaderFromDetailPage(page);
    await waitForReaderViewportImages(page);
  ...
  test('TC-065 夜间翻页主题下诗歌块通过标准富文本管线渲染正确', async ({ page }) => {
    const { novelId } = await importFixtureToDetailPage(page, 'analysisLinked');
